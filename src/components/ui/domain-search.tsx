...
const DomainSearch = ({
  ...
  return <div className="bg-black/40 backdrop-blur-sm border border-gray-800/50 rounded-xl p-8 shadow-[0_8px_30px_rgba(0,0,0,0.5)] max-w-xl mx-auto">
      ...
      <form onSubmit={handleSearch} className="flex flex-col space-y-4">
        <div className="flex items-center bg-gray-900/60 rounded-full p-1 border-[0.5px] border-white/30">
          <Input type="text" placeholder="yourname" value={subdomain} onChange={e => setSubdomain(e.target.value)} className="bg-transparent border-0 focus:ring-0 text-white flex-grow rounded-full shadow-none" autoComplete="off" />
