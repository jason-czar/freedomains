...
const DNSRecordManager: React.FC<DNSManagerProps> = ({
  ...
  const fetchDomainSettings = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("domains").select("settings").eq("id", domainId).single();
  ...
  return <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={refreshRecords} disabled={refreshing}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-4 w-full">
          <TabsTrigger value="records" className="bg-[#4d9cfb] text-slate-50 rounded-3xl ">DNS Records</TabsTrigger>
        ...
        <TabsContent value="records">
          <RecordsTabContent records={records} loading={loading} refreshing={refreshing} fullDomain={fullDomain} subdomain={subdomain} domainSuffix={domainSuffix} onRefresh={refreshRecords} />
        </TabsContent>
        
        <TabsContent value="add">
          <AddRecordTabContent fullDomain={fullDomain} subdomain={subdomain} domainSuffix={domainSuffix} adding={adding} setAdding={setAdding} existingRecords={records} onRecordAdded={refreshRecords} setActiveTab={setActiveTab} domainId={domainId} />
        </TabsContent>
        
        <TabsContent value="nameservers">
          <NameserversTabContent domainId={domainId} subdomain={subdomain} domainSuffix={domainSuffix} nameservers={nameservers} records={records} onNameserversUpdated={refreshRecords} setActiveTab={setActiveTab} />
        </TabsContent>
        
        <TabsContent value="forwarding">
          <ForwardingTabContent domainId={domainId} subdomain={subdomain} domainSuffix={domainSuffix} forwardingUrl={forwardingUrl} forwardingType={forwardingType} records={records} onForwardingUpdated={refreshRecords} setActiveTab={setActiveTab} />
        </TabsContent>
      </Tabs>
    </div>;
};
